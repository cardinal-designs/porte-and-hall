...
async function updatePagination() {
  fetch(`${window.location.href}&section_id=template--15509523529791__main`)
    ...
    .then((responseText) => {
      ...
      const selectors = ['.pagination-wrapper'];
...
const targetNode = document.querySelector('.pagination-wrapper');

// Ensure the targetNode exists before setting up the observer
if (targetNode) {
    ...
    const callback = function(mutationsList, observer) {
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList') {
                // Check if a new element with the class 'pagination' has been added
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE && node.classList.contains('pagination')) {
                        // console.log('A new .pagination element was added:', node);
                    }
                });
                let paginationContent = "";
                // Check if an element with the class 'pagination' has been removed
                ...
                if (mutation.target.classList.contains('pagination') && mutation.attributeName === 'id') {
                  updatePagination();
                    // console.log('The id of a .pagination element has changed:', mutation.target);
                }
            }
        }
    };
    ...
    console.error('The target node .pagination-wrapper was not found.');
}
...
function adjustActiveFilters() {
  ...
  for (var key in filterValues) {
    if (filterValues.hasOwnProperty(key)) {
      let shortKey = key;
      if(key?.includes('filter.v.m.custom.filters.')){
        shortKey = key?.replace('filter.v.m.custom.filters.', '');
        shortKey = 'Collection';
      }
      if(key?.includes('filter.v.m.custom.')){
        shortKey = key?.replace('filter.v.m.custom.', '');
      }
      if(key?.includes('filter.v.m.filters.')){
        shortKey = key?.replace('filter.v.m.filters.', '');
      }
      if(shortKey?.trim() == 'filters'){
        shortKey = 'Collection';
      }
...
class collectionFilters extends HTMLElement {
  ...
  reloadSections(newUrl, loadUrl) {
    ...
    // Create a map to store the filtered form data
    ...
    const cleanedSearchParams = new URLSearchParams(formData);
    for (const [key, value] of cleanedSearchParams.entries()) {
      // Check if the key is 'filter.v.m.custom.size' and the value is blank
      if (value === '') {
        cleanedSearchParams.delete(key);
      }
    }
    const params = new URLSearchParams(cleanedSearchParams?.toString());
    ...
    if (params.has('q')) {
      ...
      if(searchInput){
        let searchInputValue = searchInput.value;
        if(searchInputValue){
          let queryQ = `q=${encodeURIComponent(searchInputValue)}`;
          url = `${location.pathname}?${queryQ}&${searchParams}`;
        }
      }
      url = location.pathname + '?' + searchParams;
    }
